...
버전 업 공지는 대체로 두 종류로 나뉜다. 하나는 "기능 몇 개 추가" 느낌이고, 다른 하나는 "다음 세대 준비" 느낌인데 이번 6.0 RC는 두 번째에 해당한다. 이 글에서는 TypeScript 6.0 RC 발표 글을 기준으로, 중요한 변화만 빠르게 훑고 당장 우리 프로젝트에서 뭘 확인하면 좋은지 정리해본다.
